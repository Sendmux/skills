...
## Task 6 final native-fact adoption — 2026-09-16 06:18

M1 and the final native-producer fact checks are accepted. The candidate is locally verified, not publicly released or deployed; remaining SDK/CLI native releases, coordinated skills publication, hosted rollout, production canaries and manual Atlassian acceptance remain required. The Task5b records below retain their original inputs, counts and qualifications rather than being rewritten as later results.

- Immutable producer: published `sendmux-mcp`2.0.0, native tag `python-mcp-v2.0.0`, commit `583c81bc8467d9fc57fd9fb47c6da106ef73330b`; exact PyPI/Registry/tag and installed60-test consumer evidence is in SDK `evidence/mcp-2-release-20260916.md`. Native publication is not hosted/client certification.
- M1: only Token's `optimizing` changed to `optimising`; source/plugin SHA256 `dc71d5b01735a0a1c4f6d9577c719f8302f5411a66ea173e50db2c2091ed8458`. Exact YAML scalar SHA256 `d555bdd8ece1e6e9542f2b0d44e72e97a6ebb5d2a0a490e7e48000c47d4740e3`,951 characters/955 bytes/two newlines. Its first-iteration confirmation passed8/8 queries and24/24 decisions, with no optimiser. ROOT replayed the actual canonical classifier and rehashed300 linked files; independent review approved with qualifications. No nine-skill content/provider replay was needed.
- M1 evidence: MAIN `.claude/artifacts/mcp2-task6-token-final-confirmation-py314.UKyIdn/`; canonical result SHA256 `11c909af8b3eeb3c241ad4549aad793b04243a7a90387a09771bde0383f38727`, independent verification `dfba4840122f66b296947631bcef25443c3e577dce768fd193f33046edaf1e0c`, plus `root-adoption.json`.92 exact signed handles and24 provider workspaces were verified absent. This finite result is not future100% reliability.
- Additive M1 erratum: prepared `source_head_subtree_tree=195a1334...` copied an older HEAD's label; the correctly bound current HEAD3de00a3 Token tree is `05671bbd16cc52fc25b6b3bacb2b87b2af3c096b`. Actual HEAD/four source hashes/snapshots/one-token bytes were checked before and after execution; the original manifest remains unchanged. Two non-target native unknown-skill rejections are valid under the unchanged canonical validator, not infrastructure failures scored negative. The original-description parser's escaped representation and unrecorded launcher PGID remain qualified; actual measured scalar, exact launcher PID and terminal session were verified.
- Native facts: the real checker first rejected Go module `sendmux.ai/go/v2` against its stale unsuffixed expectation and rejected the now-released MCP2 package against old unpublished1.8 wording. The fixed Go identity, Setup paragraph, and only case7's expected output/package expectation now match the immutable producer. Protocols,54-tool split, grants, client qualifications, prompt and other criteria are unchanged. Go/v2 here is source identity, not a Go publication claim.
- Tests: +`scripts/check-skill-drift.test.mjs:rejects a stale unsuffixed Go module after accepting the v2 producer`; +`:requires consistent released package guidance in the setup guide and case 7`. Both were seen RED at the actual checker, then GREEN. The initial updated full suite passed37/37, zero skips. No tests were removed.
- Independent native-fact review found I1: `containsInOrder()` accepted expected facts inside different identity/version/count tokens. ROOT's added `rejects substring matches for published contract values and sync fields` test first exposed18 false greens: package suffixes, prerelease/longer versions, larger/fractional totals at all three publishing locations and all three sync property suffixes. The first fix left signs/exponents/ranges open; a second RED showed nine remaining false greens. The final shared boundary rejects all27 mutations, preserves valid54-tool prose and ordered matching, and covers every caller of that helper. Original REDs, failed broad-guide fixture setup and partial-fix GREENs are preserved. The fixture setup was narrowed to the actual paragraph after it incorrectly changed console-script names; no assertion was weakened.
- Final tests:38/38, failed/cancelled/skipped/todo0,12,172ms; SHA256 `3f370998e8789801c65af08534103e251a6c2a09cfc28c9c57de79e2eb93294a`. Direct pinned drift, plugin/OpenClaw bundle checks, both workflow filters and diff check passed. Final independent review reran the27-boundary public-checker test and approved with I1 closed, no remaining findings. Checker SHA256 `1dc1e1550e0541ca2d6c059d597b7a49ee3b4dd37bc6592f9ec140ba40ae2a58`; tests `f64604b1cfdf4622cd377bd09d1419e5be576b7486ad0a8760a932fa116b6236`.
- Native-fact evidence: MAIN `.claude/artifacts/mcp2-task6-native-facts.7l61BX/` contains initial REDs/37-test receipt, `exact-values-*`, `exact-numbers-*`, and ROOT preservation/adoption/cleanup receipts. Final review SHA256 `a92f5f87bd33fff7d43770a67d1a087fdc37052d59fd299c049786f4acff6d9b`. ROOT rechecked initial62 handles/11 workspaces/150 fixtures; first exact-value runs53/8/131; final numeric runs38/3/128; independent-review54 handles and seven named paths. These overlapping sets are not summed.
- Distribution: regenerated only affected derived output and rebuilt Token/Setup archives; four isolated Claude/Agents installs verified complete runtime-member byte equality and excluded evals. Token archive SHA256 `937958321db44d134b29b6d6e2c543485e28321fe90392eca344100378b0eabb`; Setup `cc055704820c9a14dbf84d4adcaf1d0b2716847635d43936d3e524e91e7783d8`. UKyIdn `root-final-package-verification.json` records27 signed handles and the install workspace absent. Prior archives are preserved; the other seven remain unchanged. The later checker-only fix changed no skill, eval, plugin or archive bytes.
- Cleanup: the owned detached SDK verification checkout `mcp2-native-pin.ZW73Zp/sdk` was removed without force after all checks/reviews finished; Git registration and exact filesystem absence were verified. Receipt: MAIN `.claude/artifacts/mcp2-native-pin.ZW73Zp/cleanup.json`. Shared immutable artifacts and native tag remain. Historical unknown-path/denied cleanup below is not silently closed by this removal.
- Skill methods: diagnosing-bugs/tdd required actual false-green regressions; writing-for-agents retained exact producer and release qualifiers; technical-documentation kept this to released facts; clean-code/refactoring kept the fix at the shared matcher rather than a new parser. No new repo-wide rule is proposed. Parked: unrelated `docs/regen-update-playbook.md` still says eight expected folders rather than nine.

## Historical Task 5b status
...
## Historical Task 5b remaining gates
...
## Historical Task 5b completion checkpoint
